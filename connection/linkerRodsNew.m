function [linkers, connection] = linkerRodsNew(rods, linkspec)
    ...
    for i = 1:npairs
        ...
        % Get rod tags
        rodATag = pairs(i, 1); rodBTag = pairs(i, 2); fprintf("Rod %d and %d: Linker - ", rodATag, rodBTag);

        % Extract rod points
        ptsA = C{rodATag}; ptsB = C{rodBTag};

        % Number of points in rod A and B
        nptsA = size(ptsA, 1); nptsB = size(ptsB, 1);

        % Find the intersecting point/s coordinate, and their row ids
        [comPts, idsA, idsB] = intersect(ptsA, ptsB, 'rows');

        % Number of common points found
