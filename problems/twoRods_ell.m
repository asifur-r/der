...
% Assign restraints
rods(1) = Restraint(rods(1), 1:2, 1:3, 1, 1); rods(1) = Restraint(rods(1), 1, 4, 1, 1);
rods(2) = Restraint(rods(2), N-1:N, 1:3, 1, 1); rods(2) = Restraint(rods(2), N-1, 4, 1, 1);
...
% rods(1) = PointLoad(rods(1), N, 3, M/25, 2); % Tip load in rod 1
rods(1) = Displacement(rods(1), N, 3, 50, 2); % Tip displacement in rod 1
...
% Time series
ana = ana.TimeSeries([Series('constant', 1), Series('linear', 1, 0, 10)]);
